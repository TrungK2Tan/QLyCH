...
class AdminOrderFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val AdminorderViewModel =
            ViewModelProvider(this).get(AdminOrderViewModel::class.java)

        _binding = FragementAdminOrderBinding.inflate(inflater, container, false)
        val root: View = binding.root

        val textView: TextView = binding.textAdminorder
        AdminorderViewModel.text.observe(viewLifecycleOwner) {
            textView.text = it
        }
        return root
    }
